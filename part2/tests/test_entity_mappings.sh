...
# Test script for entity mappings
...
SECOND_USER_ID=$(echo $SECOND_USER_RESPONSE | grep -o '"id":"[^"]*' | cut -d'"' -f4)
...
SECOND_USER_TOKEN=$(echo $SECOND_LOGIN_RESPONSE | grep -o '"access_token":"[^"]*' | cut -d'"' -f4)
